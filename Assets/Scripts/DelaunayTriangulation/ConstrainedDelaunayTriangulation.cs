...
public class ConstrainedDelaunayTriangulation
{
    ...
    private void Log(string msg) { Debug.Log($"[Constrained Delaunay Triangulation]: {msg}"); }

    public DelaunayTriangulation delaunay;

    public ConstrainedDelaunayTriangulation(TerrainInfo info, PositionSampler sampler)
    {
        this.info = info;
        this.sampler = sampler;
        this.delaunay = new DelaunayTriangulation(info, sampler);
    }

    //From the report "An algorithm for generating constrained delaunay triangulations" by Sloan
    public List<Triangle> GenerateConstrainedDelaunayTriangulation(List<Vector3> points, List<Vector3> constraints)
    {
        ...
        List<Triangle> delaunayTriangulation = delaunay.DelaunayTriangulateByFlippingEdges(points);
    ...
    private List<Triangle> AddConstraints(List<Triangle> triangulation, List<Vector3> constraints)
    {
        ...
        for (int i = 0; i < constraints.Count; i++)
        {
            ...
            Vector3 v_j = constraints[ClampListIndex(i + 1, constraints.Count)];
        ...
        //Step 5. Remove superfluous triangles (if you need to) (superfluous = unnecessary, especially through being more than enough)
    ...
    private List<HalfEdge> RemoveIntersectingEdges(Vector3 v_i, Vector3 v_j, List<HalfEdge> intersectingEdges)
    {
        ...
        while (intersectingEdges.Count > 0)
        {
            ...
            Vertex v_k = e.v;
            Vertex v_l = e.prevEdge.v;
            Vertex v_third_pos = e.nextEdge.v;
            //The vertex belonging to the opposite triangle and isn't shared by the current edge
            Vertex v_opposite_pos = e.oppositeEdge.nextEdge.v;
            ...
            if (!delaunay.IsQuadrilateralConvex(v_k.GetPos2D_XZ(), v_l.GetPos2D_XZ(), v_third_pos.GetPos2D_XZ(), v_opposite_pos.GetPos2D_XZ()))
            {
    ...
    private void RestoreDelaunayTriangulation(Vector3 v_ii, Vector3 v_jj, List<HalfEdge> newEdges)
    {

        Vertex v_i = new Vertex(v_ii);
        Vertex v_j = new Vertex(v_jj);
        ...
        while (true)
        {
            ...
            for (int j = 0; j < newEdges.Count; j++)
            {
                ...
                if ((v_k.position == v_i.position && v_l.position == v_j.position) || (v_l.position == v_i.position && v_k.position == v_j.position))
                {
                    continue;
                }
                ...
                float circleTestValue = delaunay.IsPointInsideOutsideOrOnCircle(v_k.GetPos2D_XZ(), v_l.GetPos2D_XZ(), v_third_pos.GetPos2D_XZ(), v_opposite_pos.GetPos2D_XZ());
                ...
                if (circleTestValue < 0f)
                {
                    ...
                    if (delaunay.IsQuadrilateralConvex(v_k.GetPos2D_XZ(), v_l.GetPos2D_XZ(), v_third_pos.GetPos2D_XZ(), v_opposite_pos.GetPos2D_XZ()))
                    {
                        ...
                        if (delaunay.IsPointInsideOutsideOrOnCircle(v_opposite_pos.GetPos2D_XZ(), v_l.GetPos2D_XZ(), v_third_pos.GetPos2D_XZ(), v_k.GetPos2D_XZ()) <= circleTestValue)
                        {
                            continue;
                        }
    ...
    private void RemoveSuperfluousTriangles(List<Triangle> triangulation, List<Vector3> constraints)
    {
    ...
    private bool IsAnEdgeAConstraint(Vector3 p1, Vector3 p2, List<Vector3> constraints)
    {
        for (int i = 0; i < constraints.Count; i++)
        {
            Vector3 c_p1 = constraints[i];
            Vector3 c_p2 = constraints[ClampListIndex(i + 1, constraints.Count)];
    ...
    private List<HalfEdge> FindIntersectingEdges(List<Triangle> triangulation, Vector3 p1, Vector3 p2)
    {
    ...
    private void TryAddEdgeToIntersectingEdges(HalfEdge e, Vector3 p1, Vector3 p2, List<HalfEdge> intersectingEdges)
    {
    ...
    private bool IsEdgeCrossingEdge(Vector3 e1_p11, Vector3 e1_p22, Vector3 e2_p11, Vector3 e2_p22)
    {
        Vertex e1_p1 = new Vertex(e1_p11);
        Vertex e1_p2 = new Vertex(e1_p22);
        Vertex e2_p1 = new Vertex(e2_p11);
        Vertex e2_p2 = new Vertex(e2_p22);
        ...
        if (e1_p1.position == e2_p1.position || e1_p1.position == e2_p2.position || e1_p2.position == e2_p1.position || e1_p2.position == e2_p2.position)
        {
            return false;
        }

        //Then check if the lines are intersecting
        if (!delaunay.AreLinesIntersecting(e1_p1.GetPos2D_XZ(), e1_p2.GetPos2D_XZ(), e2_p1.GetPos2D_XZ(), e2_p2.GetPos2D_XZ(), false))
        {
            return false;
        }
    ...
    private bool IsEdgePartOfTriangulation(List<Triangle> triangulation, Vector3 p1, Vector3 p2)
    {
        for (int i = 0; i < triangulation.Count; i++)
        {
